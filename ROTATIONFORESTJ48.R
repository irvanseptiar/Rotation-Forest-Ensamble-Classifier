rotFor <- function (x, y, K = round(ncol(x)/3, 0), L = 10, verbose = FALSE, 
                    ...) 
{
  library(rpart)
  library(mixOmics)
  ...
  overflow <- overflow[sample(1:length(overflow), length(overflow), 
                              FALSE)]
  subsetsizes <- sets + overflow
  if (verbose) 
    cat("Number of subsets: ", K, ". Number of variables per respective subset: ", 
        sep = "")
  if (verbose) 
    cat(subsetsizes, "\n", sep = " ")
  ...
  for (i in 1:L) {
    ...
    pcdata[[i]] <- list()
    loadings[[i]] <- list()
    varIndPerSubset <- split(sample(1:ncol(x), ncol(x), FALSE), 
                             fact)
    for (j in seq(1, K)){
      subsets[[i]][[j]] <- data.frame(x[, varIndPerSubset[[j]]], 
                                      y)
      ...
      IndependentsClassSubsetBoot[[i]][[j]] <- IndependentsClassSubset[[i]][[j]][sample(1:dim(IndependentsClassSubset[[i]][[j]])[1], 
                                                                                        ...
                                                                                        replace = TRUE), ]
      pcdata[[i]][[j]] <- prcomp(IndependentsClassSubsetBoot[[i]][[j]][, 
    ...
    Ria[[i]]$rowID <- NULL
    final <- data.frame(as.matrix(x) %*% as.matrix(Ria[[i]]), 
                        y)
    fit[[i]] <- J48(y ~ ., data = final, ..., control = Weka_control(M=1))
  }
  res <- list(models = fit, 
              ...
              pcdata = pcdata,
              Ri = Ri,
              IndependentsClassSubsetBoot = IndependentsClassSubsetBoot)
  class(res) <- "rotFor"
  res
}

prediksi <- function (object, newdata, all = FALSE, ...) 
{
