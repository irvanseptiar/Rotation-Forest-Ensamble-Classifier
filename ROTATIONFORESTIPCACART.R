rotationForestIPCACART <- function (x, y, K = round(ncol(x)/3, 0), L = 10, verbose = FALSE, ...) 
{
  library(rpart)
  library(mixOmics)
  if (K >= ncol(x)) 
  ...
  res <- list(models = fit, 
              ...
              loadings = Ria,
              pcdata = pcdata)
  class(res) <- "rotationForestIPCACART"
  res
}

predictipcacart <- function (object, newdata, all = FALSE, ...) 
{
